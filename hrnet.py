        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    upscale = int(x[i].shape[2]/self.fuse_layers[i][j](x[j]).size(2))
                    y = y + upsample_deterministic(self.fuse_layers[i][j](x[j]),upscale)
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.layer1 = self._make_layer(Bottleneck, 64, 64, 4)###4
    ...
    def forward(self, x):
        ...
        upscale = int(x[0].size(2)/x[1].size(2))
        x1 = upsample_deterministic(x[1],upscale)
        upscale = int(x[0].size(2)/x[2].size(2))
        x2 = upsample_deterministic(x[2],upscale)
        upscale = int(x[0].size(2)/x[3].size(2))
        x3 = upsample_deterministic(x[3],upscale)
