...
class AdaFace1(torch.nn.Module):
    def __init__(self,
                 embedding_size=512,
                 m=0.4,
                 h=0.333,
    ...
    def forward(self, cosine, norms, label):
        ...
        with torch.no_grad():
            ...
            self.batch_mean = mean * self.t_alpha + (1 - self.t_alpha) * self.batch_mean.to(self.device)
            self.batch_std =  std * self.t_alpha + (1 - self.t_alpha) * self.batch_std.to(self.device)
        ...
        print(m_arc.shape,g_angular.shape)
...
class AdaFace_t(Module):
    def __init__(self,
        ...
        '''print('\n\AdaFace_t with the following property')
        print('self.m', self.m)
        print('self.h', self.h)
        print('self.s', self.s)
        print('self.t_alpha', self.t_alpha)'''
    ...
    def forward(self, embbedings, norms, label):
        kernel_norm = l2_norm(self.kernel,axis=0).to(self.device)
        ...
        # g_angular
        #print(label.size()[0])
        m_arc = torch.zeros(label.size()[0], cosine.size()[1], device=self.device)
        #m_arc = torch.zeros(label.size()[0], cosine.size()[1], device='0')
        #print(m_arc.device, label[0])
        m_arc.scatter_(1, label.reshape(-1, 1), 1.0)
        g_angular = self.m * margin_scaler * -1

        #print(m_arc.shape,g_angular.shape)
        m_arc = m_arc * g_angular #80*80,80*1
        theta = cosine.acos()
        theta_m = torch.clip(theta + m_arc, min=self.eps, max=math.pi-self.eps)
        cosine = theta_m.cos()

        # g_additive
        m_cos = torch.zeros(label.size()[0], cosine.size()[1], device=self.device)
        #m_cos = torch.zeros(label.size()[0], cosine.size()[1], device='0')
...
class AdaFace(Module):
    ...
    def forward(self, embbedings, norms, label):
        kernel_norm = l2_norm(self.kernel,axis=0).to(self.device)
        #print(embbedings.shape,kernel_norm.shape)
        cosine = torch.mm(embbedings, kernel_norm)
        cosine = cosine.clamp(-1+self.eps, 1-self.eps) # for stability
        ...
        print(m_arc.shape, label.shape, label.reshape(-1, 1).shape)
        m_arc.scatter_(1, label.reshape(-1, 1), 1.0)'''
...
class ArcFace(Module):
    ...
    def __init__(self, embedding_size=512, classnum=51332,  s=64., m=0.5,d = None):
